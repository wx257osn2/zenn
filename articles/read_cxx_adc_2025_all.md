...
結論から言うと，これは現代でもコンパイラマジックを使えば可能です: `std::__is_complete_or_unbounded()` というbuiltinは直接呼び出した場合はコンパイル時でも呼び出される毎に常に最新の結果を返します^[何かメタ関数とかconstexpr関数を挟んでしまうとそのメタ関数や関数のtemplateパラメータに対して一意な結果となるようにキャッシュされてしまうので最初の呼び出しと同じ結果しか得られなくなる．だからbuiltinの呼び出しを127個並べる必要があったんですね(白目)]．
