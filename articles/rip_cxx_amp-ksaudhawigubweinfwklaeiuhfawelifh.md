...
C++ AMPが死ぬことが決まった．本稿では，C++ AMPとは何だったのか，C++ AMPの死，そしてC++ AMPの代替となりうる存在についての検討を，個人の見解として述べる．
...
4. HIP
    ...
    - 一応 `hcc` バックエンドであれば `nvcc` よりはC++対応はマシだろうが，Windowsで動作させるなら現実的には `nvcc` がバックエンドとなるため， `nvcc` の問題はすべて引きずることになる．また，Intel製GPUをサポートできない．
    - 加えて，現状AMDとしては結構HIPに力を入れているように見えるが，これがいつまで続くのかが見えない．後述するように，既にHCCを捨てたことがあるので…
5. HC(Heterogeneous Compute) C++ API
    ...
    - 今でもHIPの `hcc` バックエンドではHC C++ APIが使われているはずだが(違ったら申し訳)，一方で `hcc` 自体を直接扱うのはとうの昔にdeprecatedになっている．死にゆく言語拡張から死んだコンパイラに移行するわけないだろ！
6. SYCL
